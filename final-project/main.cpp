...
/************************ Pseudo Algorithm *****************************
 ...
 function V - Print a customerÃs details in a pag`e format to the monitor.
 function X - Exit the program.

 - NEED TO ADD A DETAILED DESCRIPTION -
...
void openFileName(string &iFileName, fstream &fin);
...
// add customer to the current file name
...
// check if it is an existing customer in the file name
Name checkCustomerName(string acctName, Customer Info[])
{
    Name cName;
    int i = countCustomer(Info);
...
int main()
{
    ...
    openFileName(iFileName, fin);
    validateFileName(fin, BankInfo, Info);
    pauseCls();
